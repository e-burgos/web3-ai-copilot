...
import { Table, TableHeader, TableRow, TableCell, Skeleton } from '@web3-ai-copilot/ui-components';
import { formatCurrency, formatNumber, formatPercentage } from '@web3-ai-copilot/shared-utils';
...
export function TokenList({ tokens: externalTokens }: TokenListProps) {
  ...
  if (displayTokens.length === 0) {
    return <p className="text-muted-foreground text-center py-8">No tokens found</p>;
  }
      ...
      <tbody>
        {displayTokens.map((token) => (
          <TableRow key={token.id}>
            ...
            <TableCell align="right">{formatNumber(parseFloat(token.balance || '0'), 4)}</TableCell>
            ...
            <TableCell>
              <div className="w-24 h-10">
                <SparklineChart data={generateSparklineData() as unknown as LineData<Time>[]} />
              </div>
            </TableCell>
          </TableRow>
        ))}
